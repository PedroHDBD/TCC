...
<body class='p-3'>
	...
	String textoAtual = (String) request.getParameter("textoAtual");
...
<div class="bg-black bg-gradient text-white p-3 rounded-3 d-flex justify-content-between align-items-center">
    <div class="d-flex align-items-center">
        <form action="./publicacoes.jsp" method="POST">
    ...
    <h5 class="m-0 position-absolute start-50 translate-middle-x text-nowrap"><%=nome%> - <%= titulo %> - Editar publicação </h5>
</h1>
...
<div class='d-flex flex-column justify-content-center align-items-center m-0 p-3 w-100'>
 		...
 		<form  action="/ProjetoTCC/api/control?acao=EditarPublicacao" method="POST" class='w-75 bg-dark-subtle py-3 px-4 rounded-3 shadow'>
			<label for="texto" class='fs-5 mb-1'>Texto: </label>
			<textarea name="texto" rows="21" maxlength="2000" value="<%=textoAtual%>" required class='w-100 lh-1 rounded-3 p-2 border border-dark border-opacity-25' placeholder='Máximo: 2000 caracteres'><%=textoAtual%></textarea>
			<input type="hidden" name="idForum" value="<%=idForum%>"> 
			<input type="hidden" name="nome" value="<%=nome%>">
			<input type="hidden" name="idTopico" value="<%=idTopico%>">
			<input type="hidden" name="idPublicacao" value="<%=idPublicacao%>"> 
			<input type="submit" name="submit" value="Editar" class='rounded-3 py-1 px-3 border border-dark border-opacity-25 mt-1 bg-secondary-subtle fs-5'>
		</form>
