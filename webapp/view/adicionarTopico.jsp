...
<body class="p-3">
    ...
    <h5 class="m-0 position-absolute start-50 translate-middle-x"><%=nome%> - Adicionar tópico </h5>
</h1>
...
<div class='d-flex flex-column justify-content-center align-items-center m-0 p-3 w-100'>
 		...
 		<form action="/ProjetoTCC/api/control?acao=AdicionarTopico" method="POST" class='w-75 bg-dark-subtle py-3 px-4 rounded-3 shadow'>
			<label for="titulo" class='fs-5 mb-1'>Título: </label>
			<input type="text" name="titulo" autocomplete="off" maxlength="60" required  class='w-100 rounded-3 py-1 px-2 border border-dark border-opacity-25' placeholder='Máximo: 60 caracteres'><br>
			<label for="texto" class='fs-5 mt-3 mb-1'>Texto: </label>
			<textarea name="texto" rows="21" maxlength="2000" required class='w-100 lh-1 rounded-3 p-2 border border-dark border-opacity-25' placeholder='Máximo: 2000 caracteres'></textarea>
			<input type="hidden" name="idForum" value="<%=idForum%>">
			<input type="hidden" name="nome" value="<%=nome%>"><br>
			<input type="submit" name="submit" value="Criar" class='rounded-3 py-1 px-3 border border-dark border-opacity-25 mt-1 bg-secondary-subtle fs-5'>
		</form>
